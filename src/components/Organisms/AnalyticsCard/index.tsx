  ...
  return (
    <div className="bg-white rounded-xl shadow-md p-6 w-full sm:w-1/2 lg:w-1/4 md:h-1/4 transition transform hover:scale-[1.01]">
      <div className="flex items-center justify-between">
        <div className={`w-12 h-12 rounded-full flex items-center justify-center text-xl ${positive ? 'bg-green-100 text-blue-600' : 'bg-red-100 text-red-600'}`}>
          {icon}
        </div>
        <div className="text-right">
          <p className="text-sm text-gray-500">{title}</p>
          <h3 className="text-2xl font-bold text-gray-800">{value.toLocaleString()}</h3>
          <p className={`text-sm mt-1 font-medium ${positive ? 'text-blue-500' : 'text-red-500'}`}>
